...
export class DevelopmentDiscoveryProvider implements IDiscoveryProvider {
  ...
  public async search(request: DiscoverySearchRequest): Promise<DiscoverySearchResult> {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      const locData = this.synthesizeLocationData(locationStr, i);

      let emailUser: string;
      if (parsed.entityType === 'person') {
        const fName = personalFirstNames[i % personalFirstNames.length];
        const lName = personalLastNames[i % personalLastNames.length];
      ...
      prospects.push({
        id: `disc_${Date.now().toString(36)}_${i}_${Math.random().toString(36).substring(2, 7)}`,
        name,
        entityType,
